...
----------------------------------
	...
	events.PLAYER_LOGIN = function()
		...
		if db.init ~= 2 or not StufRaidCharDB then
			LoadAddOn("StufRaid_Options")
		...
		db.raid.group11 = db.raid.group11 or { hide=true, classorder="MONK", x=-340, y=-7, growth="TBLR", cols=1, rows=5, }  -- 11
		db.raid.group12 = db.raid.group12 or { hide=true, classorder="DEMONHUNTER", x=-373, y=-7, growth="TBLR", cols=1, rows=5, }  -- 12
		db.raid.group9.order = db.raid.group9.order or "9"
		db.raid.group10.order = db.raid.group10.order or "10"
		db.raid.group11.order = db.raid.group11.order or "11"
		db.raid.group12.order = db.raid.group12.order or "12"
