...
local function update_diseases()
	...
	i = 1;
	while true do
		local name, _, _, _, _, dur, exp, _, _, _, id = UnitDebuff("target", i, "PLAYER");
		if name then
...
local function update_tracking()
	...
	while true do
		if track_target == "target" then
			name, _, icon, count, _, dur, exp, _, _, _, id = UnitDebuff(track_target, i);
		else
			name, _, icon, count, _, dur, exp, _, _, _, id = UnitBuff(track_target, i);
		end
